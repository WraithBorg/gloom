    ...
    private DataSource initDDS(String projectCode) throws IllegalAccessException {
        ...
        for (Field f : pfields) {
            ...
            try {
				...
				// Can not set static final long field org.apache.tomcat.jdbc.pool.PoolProperties.serialVersionUID to java.lang.Long
            }
        }
        dds.setPoolProperties(property);
        String urlFormat = this.getUrl();
        String url = String.format(urlFormat,
                ProjectDBMgr.instance().getDBIP(projectCode),
